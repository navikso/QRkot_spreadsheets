...
SPREADSHEET = {
    "properties": {"title": settings.report_title, "locale": "ru_RU"},
    "sheets": [TABLE],
}
...
async def set_user_permissions(
    ...
    await wrapper_service.as_service_account(
        service.permissions.create(fileId=spreadsheet_id, json=USER_DATA, fields="id")
    )
...
async def spreadsheets_update_value(
    spreadsheet_id: str, charityprojects: list, wrapper_services: Aiogoogle
) -> None:
...
async def get_spreadsheets_from_disk(
    spreadsheet_title: str, wrapper_service: Aiogoogle
) -> List[Dict[str, str]]:
    service = await wrapper_service.discover(
